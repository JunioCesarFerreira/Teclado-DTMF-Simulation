...
// Atribuição pinos de controle do LCD
...
#define FS 16000                // Frequência de amostragem em Hz
#define TS 60                   // Tempo de amostragem em us

// Variaveis globais
unsigned short v[N];            // Vetor receberá o sinal amostrado
double FAG[8];                  // Vetor receberá as componentes espectrais resultantes
char txt[7] = {0,0,0,0,0,0,0};  // String para textos de Debug

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  Função computa o algoritmo de Goertzel
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
double Goertzel_Algorithm(double Fr){
       // Fr é a frequência desejada, retorna a magnetude desta frequência.
       // Constantes
       double w;
       double cosw;
       double sinw;
       ...
       z1=(z2*sinw)/N;           // Componente imaginária
       z2=sqrt(z0*z0+z1*z1);     // Magnetude
       return z2;
}

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  Função de detecção das teclas precionadas por análise espectral
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
char Detect_key(){
     unsigned short j;     // Indexador
     double Max=0;         // Variavel auxiliar para encontrar os máximos
     char Ind1=0, Ind2=0;  // Indexadores dos máximos
     // Encontra indeces dos pontos máximos do espectro
     for (j = 0; j < 8; j++){
         if (FAG[j] > Max){
            Max = FAG[j];
            Ind1 = j;
     ...
     // Realiza filtragem
     Ind2*=10;     // Para que Ind2 corresponda a um valor válido ele deve ser ou 4,5,6,7,
     Ind1 += Ind2; // Compõe um número decimal para verificação
     switch (Ind1){
              case 40:
                  return '1';
              case 50:
     ...
     return ' ';  // Caso a verificação falhe retorna espaço
}
...
void main() {
     ...
     char Resultado;                     // Receberá o resultado da filtragem
     bit Data_Received;                  // Flag indicador de dado amostrado
     // Configura pinos de I/O
     TRISB = 0; // Todos como saídas
     ...
     Lcd_Out(2,1,">");
     // Inicializa UART para debug na simulação
     UART1_Init(9600);
     // Inicializa indicando que nenhuma amostra está disponível
     Data_Received=0;
     do{
        ...
        if (Data_Received){
           ...
           Data_Received=0; // Indica que dados amostrais já foram processados
           // *** Apenas para Debug em simulação
